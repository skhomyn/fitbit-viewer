...
/**
 * Java Object which passes information from the the <code>DailyRecord</code>
 * object to the <code>InterfaceView</code> object.
 * <p>
 * <code>DailyDashboardController</code> contains references to the model object
 * {@link DailyRecord}, which contains the data for the Best Days page, and the
 * view object {@link InterfaceView}, which implements the user interface for
 * all pages.
 * <p>
 * The purpose of this class is to pass the data contained in the model
 * <code>DailyRecord</code> to the view <code>InterfaceView</code>, so that it
...
public class DailyDashboardController {
	 ...
	 * {@link InterfaceView#addClickListenerDashboard(ActionListener)} to add a
	 * click listener to the Daily Dashboard button.
	 * 
	 * @param model
	...
	/**
	 * Local class accessible only by the <code>DailyDashboardController</code>
	 * object. It's purpose is to define which actions should be performed when
	 * the Daily Dashboard button in <code>InterfaceView</code> is clicked by
	 * the user
	 *
	...
	class clickListener implements ActionListener {
		 ...
		 * Sets the Daily Dashboard page display fields in
		 * <code>InterfaceView</code> with values from the appropriate model
		 * <code>DailyRecord</code> fields upon the click of a button.
		 * 
		 * @param e
